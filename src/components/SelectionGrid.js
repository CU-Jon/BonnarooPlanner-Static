...
export default function SelectionGrid({ data, type, onToggleSelection, currentSelections }) {
  return (
              ...
              return (
                <div className="location-block" key={loc}>
                  ...
                  {sorted.map(ev => {
                    ...
                    const isChecked = currentSelections.some(sel =>
                      sel.type === type && sel.day === day && sel.location === loc && sel.event.name === ev.name
                    );
                    return (
                      <label key={ev.name}>
                        <input
