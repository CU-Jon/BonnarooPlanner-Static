...
function buildSelectionUI() {
    ...
    // After rebuilding UI, activate the first tab.
    // Use setTimeout to ensure the DOM update is processed before the click event fires.
    setTimeout(() => {
...
function openTab(evt, tabName) {
    // Clear previous selections within the currently active tab (if any)
    // This logic might need refinement depending on desired behavior when switching tabs
    // document.querySelectorAll(".tabcontent[style*='block'] input[type='checkbox']").forEach(cb => cb.checked = false); // Or maybe clear all checkboxes?
    // For simplicity, let's stick to the original logic for now which clears all checkboxes on any tab switch:
    document.querySelectorAll("input[type='checkbox']").forEach(cb => cb.checked = false);


    // Hide all tab content divs
    document.querySelectorAll('.tabcontent').forEach(e => {
        if (e) { // Add null check for safety
    ...
    // Find the target tab content div
    ...
    if (tabElement) {
        ...
        // Log a warning if the element isn't found - might happen if clicked too early
        console.warn(`Tab content element with ID "${tabName}" not found during openTab call.`);
    }

    // Activate the clicked/corresponding tab link button
    // evt might be missing or different during programmatic click
    if (evt && evt.currentTarget) {
        // Called by a user click or a well-formed programmatic click event
        evt.currentTarget.classList.add('active');
    } else {
        // If called programmatically without a proper event, find the button manually
        document.querySelectorAll('.tablinks').forEach(button => {
            const onclickAttr = button.getAttribute('onclick');
            // Check if the onclick attribute contains the call to openTab with the correct tabName
